...
%--------------------------目标函数
obj = @(x) leader_objective(x, idx, env);
...
function val = leader_objective(x, idx, env)
    
    PN = x(idx.P)';
    PJ = x(idx.Po)';
    %--------记录迭代过程
%     res.count = res.count + 1;
%     res.PJ1_T(res.count) = PJ(1)
%     res.PJ2_T(res.count) = PJ(2);
%     res.PN1_T(res.count) = PN(1);
%     res.PN2_T(res.count) = PN(2);
%     res.PN4_T(res.count) = PN(3);
    
    I = I_calcu(PN, PJ);
    [RN, uN] = uN_calcu(PN, I);
    uo = -sum(uN) - env.c_o * sum(PJ);
    val = -uo;
end
...
function [c_neq, c_eq] = followers_kkt_constraints(x, idx, env, Loss)
    ...
    df_dp = [];
    for i = 1:N
        dfi_dpi = -env.Bandwidth / (PN(i) + Loss(i, i) * ( env.sigma2 + I(i))) + env.c_f;
        df_dp = [df_dp; dfi_dpi];
    end
    % 暂时只考虑一个上界，不考虑mu
...
%         fprintf('Iteration %d: p_o = [%s], p_f = [%s]\n', optimValues.iteration, mat2str(p_o), mat2str(p_f));
    ...
    if isequal(state, 'done')
        % 在优化结束时绘制 p_o 的历史变化图
        figure(2);
        plot(1:length(PN1), PN1,  'LineWidth',1.2, 'Marker', '^', 'DisplayName', 'N1', 'MarkerIndices', 1:20:length(PN1)); hold on;
        plot(1:length(PN2), PN2,  'LineWidth',1.2, 'Marker', 'd', 'DisplayName', 'N2', 'MarkerIndices', 1:20:length(PN2));
        plot(1:length(PN3), PN3,  'LineWidth',1.2, 'Marker', 's', 'DisplayName', 'N3', 'MarkerIndices', 1:20:length(PN3));
        
        plot(1:length(PJ1), PJ1,  'LineWidth',1.2, 'Marker', 'o', 'DisplayName', 'J1', 'MarkerIndices', 1:20:length(PJ1));
        plot(1:length(PJ2), PJ2,  'LineWidth',1.2, 'Marker', 'x', 'DisplayName', 'J2', 'MarkerIndices', 1:20:length(PJ2));
